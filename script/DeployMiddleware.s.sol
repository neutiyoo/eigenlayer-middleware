...
contract DeployMiddleware is Script {
    ...
    function doIt() external {
        ...
        for (uint256 i = 0; i < numOperators; i += batchSize) {
            ...
            for (uint256 j = i; j < endIndex; j++) {
                ...
                console.log(operators[j].key.addr);
    ...
    function doIt_ForReal() external {
        // Deploy multiple tokens and strategies first
        console.log("Deploying tokens and strategies...");
        tokens = new address[](NUM_STRATEGIES);
        strategies = new address[](NUM_STRATEGIES);
        ...
        // Enable operator sets
        vm.startBroadcast(config.admin); /// Should be deployer
        RegistryCoordinator(middlewareDeployment.registryCoordinator).enableOperatorSets();
        vm.stopBroadcast();
        console.log("Operator sets enabled");
        ...
        for (uint256 i = 0; i < numOperators; i += batchSize) {
            ...
            for (uint256 j = i; j < endIndex; j++) {
                ...
                // Add balance using tenderly RPC
                string memory params = string(abi.encodePacked("[\"", vm.toString(operators[j].key.addr), "\",\"0xDE0B6B3A7640000\"]")); // 1 ether in hex
                vm.rpc("tenderly_addBalance", params);

                vm.startBroadcast(operators[j].key.privateKey);
                OperatorLib.registerAsOperator(operators[j], core.delegationManager);
                vm.stopBroadcast();
            }
        ...
        // Use tenderly RPC to increase blocks
        vm.rpc("evm_increaseBlocks", "\"0x64\""); // Increase by 100 blocks (0x64 in hex)
        ...
        // Use tenderly RPC to increase blocks
        vm.rpc("evm_increaseBlocks", "\"0x0A\""); // Increase by 10 blocks (0x0A in hex)
